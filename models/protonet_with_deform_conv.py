...
class PrototypicalNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Replace existing deform layers initialization with conditional one
        if config.use_deform_conv:
            self.deform_layers = nn.ModuleList([
                DeformableConv2d(
                    self.backbone_dim,
                    self.backbone_dim,
                    kernel_size=config.deform_kernel_size
                )
                for _ in range(config.num_deform_layers)
            ])
        else:
            self.deform_layers = nn.ModuleList([])  # Empty list if not using deform conv
    ...
    def forward_features(self, x):
        ...
        # Replace existing deform conv application with conditional one
        if self.config.use_deform_conv:
            for deform_layer in self.deform_layers:
                x = F.relu(deform_layer(x))

        # Add proper flattening after global pooling
